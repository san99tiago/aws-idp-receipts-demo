...
from aws_cdk import (
    ...
    aws_iam,
    aws_lambda,
    aws_lambda_event_sources as l_event_sources,
    aws_logs,
    aws_s3,
...
class BackendStack(Stack):
    ...
    def create_sqs(self) -> None:
        """
        Method to create the SQS Queue for the API.
        """

        # Create SQS and DQL for the PDF processing
        self.sqs_queue_dlq = aws_sqs.DeadLetterQueue(
            max_receive_count=3,
            queue=aws_sqs.Queue(
                self,
                "SQS-QueueDLQ",
                queue_name=f"{self.main_resources_name}-sqs-queue-dlq",
        ...
        self.sqs_queue = aws_sqs.Queue(
            self,
            "SQS-Queue",
            queue_name=f"{self.main_resources_name}-sqs-queue",
            visibility_timeout=Duration.seconds(30),
            encryption=aws_sqs.QueueEncryption.SQS_MANAGED,
            enforce_ssl=True,
        )
    ...
    def create_lambda_functions(self) -> None:
        ...
        self.lambda_documents_api: aws_lambda.Function = aws_lambda.Function(
            ...
            environment={
                ...
                "TABLE_NAME": self.dynamodb_table.table_name,
                "S3_BUCKET_NAME": self.s3_bucket_receipts.bucket_name,
                "SQS_URL_AFTER_IDP_PROCESSING": self.sqs_queue.queue_url,
            },
            layers=[
                self.lambda_layer_powertools,
                self.lambda_layer_common,
        ...
        self.sqs_queue.grant_send_messages(self.lambda_documents_api)

        # Lambda Function for creating the output PDF documents after final IDP processing
        self.lambda_idp_after_process: aws_lambda.Function = aws_lambda.Function(
            self,
            "Lambda-IDPAfterProcess",
            runtime=aws_lambda.Runtime.PYTHON_3_11,
            function_name=f"{self.main_resources_name}-after-process",
            description=f"Function after IDP process to create the output PDF documents for {self.main_resources_name}",
            handler="after_idp/lambda_function.lambda_handler",
            code=aws_lambda.Code.from_asset(PATH_TO_LAMBDA_FUNCTION_FOLDER),
            timeout=Duration.seconds(30),
            memory_size=512,
            environment={
            ...
            events=[
                l_event_sources.SqsEventSource(
                    self.sqs_queue,
                    batch_size=1,  # Intentionally set to 1 for now...
                )
            ],
        )
        self.dynamodb_table.grant_read_write_data(self.lambda_idp_after_process)
        self.s3_bucket_receipts.grant_read_write(self.lambda_idp_after_process)
    ...
    def configure_rest_api(self):
        ...
        root_resource_docs: aws_apigw.Resource = self.root_resource_v1.add_resource(
            "docs",
